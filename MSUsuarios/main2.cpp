...
const std::string conn_str = "host=postgres-container port=5432 dbname=traccar_db user=postgres password=1234";
...
bool check_database_connection() {
    ...
    try {
        ...
        if (!C.is_open()) {
            ...
            pqxx::work W(C);
            pqxx::result R = W.exec("SELECT to_regclass('public.users')");
            if (R[0][0].is_null()) {
                std::cerr << "Error: La tabla 'users' no existe en la base de datos." << std::endl;
                result = false;
            } else {
                std::cout << "Conexión a la base de datos exitosa y la tabla 'users' está disponible." << std::endl;
                result = true;
            }
        }
...
std::string get_users_from_db() {
    ...
    try {
        ...
        for (size_t i = 0; i < R.size(); i++) {
            ...
            user_info += "Username: " + std::string(row["username"].c_str()) + ", ";
...
std::string create_user(const crow::json::rvalue& json) {
    ...
    try {
        if (!json.has("username") || !json.has("email") || !json.has("password")) {
            ...
            pqxx::connection C(conn_str);
            pqxx::work W(C);
            W.exec("INSERT INTO users (username, password, email, token) VALUES (" + 
    ...
    CROW_ROUTE(app, "/users/login").methods(crow::HTTPMethod::POST)([](const crow::request& req) {
        ...
        if (!json || !json.has("email") || !json.has("password")) {
            ...
            try {
                ...
                pqxx::result R = W.exec("SELECT token, password FROM users WHERE email = " + W.quote(email));
