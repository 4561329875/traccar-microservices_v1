...
char* actuaConductor(PGconn *conn, char *parametrosYvalores, char *selecion) {
    size_t query_len = strlen("UPDATE tc_drivers SET ") + strlen(parametrosYvalores) + strlen(" WHERE ") + strlen(selecion) + strlen(" RETURNING id  ") + 1;
    ...
    strcpy(query, "UPDATE tc_drivers SET ");
